...
function DRKLib:CreateAboutTab(config)
  ...
  local function getInfo()
    ...
    return
      ...
      "Script: " .. tostring(config.ScriptName or "DRK Hub") .. "\n" ..
      "Exploit: " .. exploit .. "\n" ..
      "Ping: " .. ping .. " ms\n" ..
      "FPS: " .. fps .. "\n" ..
  ...
  about:AddTextLabel(
    ...
    "Discord: " .. tostring(config.Discord or "Not Set") .. "\n" ..
    "GitHub: " .. tostring(config.GitHub or "Not Set")
  )
  about:AddTextLabel("Changelog\n" .. tostring(config.Changelog or "UI improvements and fixes"))

  local function addCopyButton(title, value)
    if value and value ~= "" and value ~= "Not Set" then
      about:AddButton(title, function()
        if setclipboard then
          pcall(setclipboard, value)
          ShowNotification(self.ScreenGui, title .. " copied", Configs_HUB.Cor_Stroke)
        end
      end)
    end
  end

  addCopyButton("Discord", config.Discord)
  addCopyButton("GitHub", config.GitHub)

  task.spawn(function()
    while infoLabel.Frame and infoLabel.Frame.Parent do
      task.wait(1)
