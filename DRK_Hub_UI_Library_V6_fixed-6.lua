...
function DRKLib:SetThemeColor(newColor)
  ...
  local function inColorPicker(obj)
    local current = obj
    while current do
  ...
  local function updateStroke(stroke, allowColorPicker)
    if not stroke or not stroke.Parent then
      return
    end

    local picker = inColorPicker(stroke)
    if picker and not allowColorPicker then
      return
    end

    if stroke:GetAttribute("DRK_CustomStroke") then
      return
    end
  ...
  for _, stroke in ipairs(ActiveStrokes) do
    local allowColorPicker = stroke:GetAttribute("DRK_ColorPickerThemeStroke") == true
    updateStroke(stroke, allowColorPicker)
  end

  if CurrentWindowRef and CurrentWindowRef.Parent then
    for _, obj in ipairs(CurrentWindowRef:GetDescendants()) do
      local isPicker = inColorPicker(obj)

      if obj:IsA("UIStroke") then
        if isPicker and obj:GetAttribute("DRK_ColorPickerThemeStroke") == true then
          updateStroke(obj, true)
        end
      elseif not isPicker then
        if obj:IsA("Frame") or obj:IsA("TextButton") or obj:IsA("TextLabel") or obj:IsA("ImageButton") or obj:IsA("ImageLabel") or obj:IsA("ScrollingFrame") or obj:IsA("TextBox") then
        ...
        if obj:IsA("ImageLabel") or obj:IsA("ImageButton") then
          if obj.ImageColor3 == oldColor then
            obj.ImageColor3 = newColor
          end
        end

        if obj:IsA("TextBox") and obj.PlaceholderColor3 == oldColor then
          obj.PlaceholderColor3 = newColor
        end

        if obj:IsA("ScrollingFrame") and obj.ScrollBarImageColor3 == oldColor then
          obj.ScrollBarImageColor3 = newColor
        end

        if obj:IsA("UIGradient") then
          local points = obj.Color.Keypoints
          local rebuilt = table.create(#points)
          local changed = false
          ...
          for i, point in ipairs(points) do
            ...
            rebuilt[i] = ColorSequenceKeypoint.new(point.Time, color)
          end
