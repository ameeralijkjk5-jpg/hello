...
function DRKLib:CreateAboutTab(config)
  ...
  about:AddImageLabel({
    Name = config.ImageName or "صورة اللاعب",
    Image = image or ""
  })
  ...
  local infoLabel = about:AddTextLabel(buildInfo())

  about:AddSection(config.AboutSection or "About")

  local aboutText = config.AboutText

  if aboutText == nil then
    local parts = {}

    if config.Name ~= nil then
      table.insert(parts, "Name: " .. tostring(config.Name))
    end

    if config.Version ~= nil then
      table.insert(parts, "Version: " .. tostring(config.Version))
    end

    if config.Developer ~= nil then
      table.insert(parts, "Developer: " .. tostring(config.Developer))
    end

    aboutText = table.concat(parts, "\n")
  end

  if tostring(aboutText or "") ~= "" then
    about:AddTextLabel(tostring(aboutText))
  end
  ...
  if config.Discord and config.Discord ~= "" then
    about:AddLinkButton("Discord", config.Discord, config.DiscordIcon)
  end

  if config.GitHub and config.GitHub ~= "" then
    about:AddLinkButton("GitHub", config.GitHub, config.GitHubIcon)
  end
  ...
  local updateInfo = type(config.GetInfo) == "function" or config.AutoUpdateInfo == true
