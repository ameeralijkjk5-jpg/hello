...
function DRKLib:CreateTab(tabName, tabIcon)
    ...
    function TabFunctions:AddTextLabel(text)
        ...
        local height = math.clamp(26 + ((lines - 1) * 16), 36, 180)
        ...
        function api.Update(newText)
            newText = tostring(newText or "")
            local count = 1
            for _ in newText:gmatch("\n") do count = count + 1 end
            local newHeight = math.clamp(26 + ((count - 1) * 16), 36, 180)
            label.Text = newText
...
function DRKLib:CreateAboutTab(config)
    ...
    local tab = self:CreateTab(config.TabName or "About", config.TabIcon)

    if config.Image and tostring(config.Image) ~= "" then
        local imageFrame = Create("Frame", tab._Tab, {
            BackgroundColor3 = Configs_HUB.Cor_Options,
            Size = UDim2.new(1, -16, 0, 190),
            ZIndex = 5
        })
        Corner(imageFrame)
        Stroke(imageFrame, {Thickness = 1})
        Create("TextLabel", imageFrame, {
            BackgroundTransparency = 1,
            Position = UDim2.new(0, 12, 0, 7),
            Size = UDim2.new(1, -24, 0, 22),
            Font = Configs_HUB.Text_Font,
            Text = tostring(config.ImageName or "Player Image"),
            TextColor3 = Configs_HUB.Cor_Text,
            TextSize = 11,
            TextXAlignment = Enum.TextXAlignment.Left,
            ZIndex = 6
    ...
    tab:AddSection(config.InfoSection or "Information")

    local infoLabel
    local function getInfo()
        if type(config.GetInfo) == "function" then
            local ok, result = pcall(config.GetInfo)
            if ok then
                return tostring(result or "")
            end
        end
        return tostring(config.Info or "")
    end

    infoLabel = tab:AddTextLabel(getInfo())

    tab:AddSection(config.AboutSection or "About")

    if config.AboutText ~= nil then
        tab:AddTextLabel(tostring(config.AboutText))
    else
        local parts = {}
        if config.Name ~= nil then table.insert(parts, "Name: " .. tostring(config.Name)) end
        if config.Version ~= nil then table.insert(parts, "Version: " .. tostring(config.Version)) end
        if config.Developer ~= nil then table.insert(parts, "Developer: " .. tostring(config.Developer)) end
        if #parts > 0 then tab:AddTextLabel(table.concat(parts, "\n")) end
    end

    if config.Changelog ~= nil and tostring(config.Changelog) ~= "" then
        tab:AddTextLabel("Changelog\n" .. tostring(config.Changelog))
    end

    if type(config.Links) == "table" then
        for _, link in ipairs(config.Links) do
            if type(link) == "table" and link.Name and link.Url then
                tab:AddButton(tostring(link.Name), function()
                    local url = tostring(link.Url)
                    local opened = false
                    pcall(function()
    ...
    if type(config.GetInfo) == "function" then
        task.spawn(function()
            while infoLabel and infoLabel.Frame and infoLabel.Frame.Parent and self.ScreenGui and self.ScreenGui.Parent do
                task.wait(1)
